...
const createInvoiceSchema = z.object({
  ...
  patientCreditBalance: z.number().optional().default(0),
  creditBalanceDate: z.string().transform((val) => new Date(val)).optional(),
  extraNotes: z.string().optional().default(""),
});

// Update schema (same as create but all fields optional except invoiceNumber)
...
export async function registerRoutes(
  ...
  app.put("/api/invoices/:id", async (req, res) => {
    try {
      ...
      // Handle creditBalanceDate conversion if provided
      if (updates.creditBalanceDate && typeof updates.creditBalanceDate === 'string') {
        updates.creditBalanceDate = new Date(updates.creditBalanceDate);
      }
