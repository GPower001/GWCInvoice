    ...
    <Layout>
      <div className="space-y-8 pb-8">
        ...
        <div className="relative overflow-hidden bg-gradient-to-br from-amber-50 via-white to-amber-50/30 rounded-2xl p-8 border-2 border-amber-200/50 shadow-lg">
          <div className="absolute top-0 right-0 w-96 h-96 bg-gradient-to-br from-amber-400/10 to-transparent rounded-full blur-3xl"></div>
          <div className="relative flex flex-col md:flex-row md:items-center justify-between gap-4">
            <div>
              ...
              <p className="text-amber-800/70 font-medium">Overview of your invoice statistics</p>
            </div>
            
            <div className="flex flex-wrap items-center gap-3">
              <div className="flex items-center bg-white rounded-xl border-2 border-amber-300/50 p-1.5 shadow-sm">
                <Button 
                  variant={currency === "NGN" ? "default" : "ghost"} 
                  size="sm" 
                  onClick={() => setCurrency("NGN")}
                  className={`h-8 px-3 ${currency === "NGN" ? 'bg-gradient-to-r from-amber-500 to-amber-600 text-white shadow-md' : 'hover:bg-amber-50'}`}
                >
                  NGN
                </Button>
                <Button 
                  ...
                  size="sm" 
                  onClick={() => setCurrency("USD")}
                  className={`h-8 px-3 ${currency === "USD" ? 'bg-gradient-to-r from-amber-500 to-amber-600 text-white shadow-md' : 'hover:bg-amber-50'}`}
                >
                  USD
                </Button>
              </div>
              ...
              {currency === "USD" && (
                <div className="flex items-center gap-2 border-2 border-amber-300/50 rounded-xl px-3 bg-white h-10 shadow-sm">
                  <span className="text-xs text-amber-700 whitespace-nowrap font-semibold">Rate: ₦</span>
                  <input 
                    type="number" 
                    value={rate} 
                    onChange={(e) => setRate(Number(e.target.value))}
                    className="w-16 bg-transparent border-none focus:ring-0 text-sm font-bold text-amber-900"
                  />
                </div>
              )}

              <Select value={duration} onValueChange={setDuration}>
                <SelectTrigger className="w-[150px] border-2 border-amber-300/50 bg-white">
                  <SelectValue placeholder="Duration" />
                </SelectTrigger>
                <SelectContent>
                  <SelectItem value="7">Last 7 Days</SelectItem>
              ...
              <Button onClick={handleDownloadReport} className="gap-2 bg-gradient-to-r from-amber-500 to-amber-600 hover:from-amber-600 hover:to-amber-700 text-white shadow-lg shadow-amber-500/30">
                <Download className="w-4 h-4" />
                Download Report
              </Button>
            </div>
        ...
        <div className="grid gap-6 md:grid-cols-2 lg:grid-cols-4">
          {stats.map((stat) => (
            <Card key={stat.title} className="border-2 border-amber-200/50 bg-gradient-to-br from-white via-amber-50/10 to-white shadow-lg hover:shadow-xl transition-all duration-300 group">
              <CardHeader className="flex flex-row items-center justify-between pb-2">
                <CardTitle className="text-sm font-semibold text-amber-900/70 uppercase tracking-wider">
                  {stat.title}
                </CardTitle>
                <stat.icon className={`h-5 w-5 ${stat.color}`} />
              </CardHeader>
              <CardContent>
                <div className="text-3xl font-bold bg-gradient-to-r from-amber-700 to-amber-600 bg-clip-text text-transparent">
                  {stat.value}
                </div>
        ...
        <div className="grid gap-6 md:grid-cols-2 lg:grid-cols-3">
          <Card className="lg:col-span-2 border-2 border-amber-200/50 bg-gradient-to-br from-white via-amber-50/10 to-white shadow-lg">
            <CardHeader>
              <CardTitle className="text-xl font-bold text-amber-900">Revenue Overview</CardTitle>
            </CardHeader>
            <CardContent>
              <div className="h-[300px] w-full">
                <ResponsiveContainer width="100%" height="100%">
                  <BarChart data={monthlyRevenue}>
                    ...
                    <Tooltip 
                      formatter={(value: number) => [`${currencySymbol}${value.toLocaleString()}`, "Income"]}
                      contentStyle={{ backgroundColor: '#FFFBEB', border: '2px solid #FCD34D', borderRadius: '8px' }}
                    />
                    <Bar dataKey="income" fill="url(#goldGradient)" radius={[8, 8, 0, 0]} />
                    <defs>
          ...
          <Card className="border-2 border-amber-200/50 bg-gradient-to-br from-white via-amber-50/10 to-white shadow-lg">
            <CardHeader>
              <CardTitle className="text-xl font-bold text-amber-900">Services Revenue</CardTitle>
            </CardHeader>
            <CardContent>
              <div className="h-[300px] w-full">
                <ResponsiveContainer width="100%" height="100%">
        ...
        <div className="grid gap-6 md:grid-cols-2">
          <Card className="border-2 border-amber-200/50 bg-gradient-to-br from-white via-amber-50/10 to-white shadow-lg">
            ...
            <CardContent>
              <div className="space-y-4">
                {topClients.map((client) => (
                  <div key={client.name} className="flex items-center justify-between p-3 rounded-lg hover:bg-amber-50/50 transition-all group border border-transparent hover:border-amber-200">
                    <div>
                      <p className="font-semibold text-gray-900">{client.name}</p>
                      <p className="text-xs text-amber-700/60 font-medium">{client.count} Invoices</p>
                    </div>
                    <div className="text-right">
                      <p className="font-bold text-lg bg-gradient-to-r from-amber-700 to-amber-600 bg-clip-text text-transparent">
                        {currencySymbol}{convertAmount(client.total).toLocaleString()}
                      </p>
                    </div>
                  </div>
                ))}
                {topClients.length === 0 && (
                  <p className="text-center text-amber-700/60 py-8 font-medium">No data available</p>
                )}
              </div>
            </CardContent>
          </Card>
          ...
          <Card className="border-2 border-amber-200/50 bg-gradient-to-br from-white via-amber-50/10 to-white shadow-lg">
            ...
            <CardContent>
              <div className="space-y-4">
                {recentInvoices.map((inv) => (
                  <div key={inv._id} className="flex items-center justify-between p-3 rounded-lg hover:bg-amber-50/50 transition-all group border border-transparent hover:border-amber-200">
                    <div>
                      <p className="font-semibold text-gray-900">#{inv.invoiceNumber}</p>
                      <p className="text-xs text-amber-700/60 font-medium">{inv.clientName}</p>
                    </div>
                    <div className="flex flex-col items-end gap-1">
                      <Badge 
                        className={`font-semibold shadow-sm ${
                          inv.status === "paid" ? 'bg-gradient-to-r from-green-500 to-emerald-600 text-white border-0' : 
                          inv.status === "overdue" ? 'bg-gradient-to-r from-red-500 to-rose-600 text-white border-0' : 
                          'bg-gradient-to-r from-amber-400 to-amber-500 text-white border-0'
                        }`}
                      >
                        {inv.status.charAt(0).toUpperCase() + inv.status.slice(1)}
                      </Badge>
                      <p className="text-sm font-bold bg-gradient-to-r from-amber-700 to-amber-600 bg-clip-text text-transparent">
                        {(inv.currency === 'NGN' ? '₦' : '$')}{convertAmount(Number(inv.total) || 0).toLocaleString()}
                      </p>
                    </div>
                  </div>
